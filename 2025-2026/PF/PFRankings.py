...
def write_to_csv(elosList):
    ...
    for team, eloSchool in elosList:
        ...
        if (members[0].strip() in nsdLastNames and members[1].strip() in nsdLastNames) or (
            school in nsdTeams
        ):
            nsd = "Y"
        else:
        ...
        dd = "Y" if school in ddTeams or ("DebateDrills" in school) else "N"
    ...
    with open("PFRankings_top500.csv", "w") as fp:
...
add_tournament("SaintsClassic", 1)
add_tournament("ChurchillClassic", 1)
add_tournament("ArizonaState", 8)
add_tournament("PatriotGames", 1)
...
add_tournament("UpperStClair-ThreeRivers", 1)
